...
1. **Put one screen in front of everyone.** A laptop on the coffee table, or
   ...
   four-letter room code, and it is the only thing that makes noise. The key
   beside *Start game* sets the difficulty: *easy* draws songs from the two
   most recent decades, *medium* from four and *hard* from six. Those are
   presets, so any decade back to the 1960s can be switched on or off by hand.
